...
class Navmap:
    ...
    def displayInfocard(self, subject: str):
        ...
        if subject in self.searchableEntities:
            infocard = self.searchableEntities[subject].infocard().strip()
            infocard = infocard.rpartition('<p>')[0]  # bit hacky, remove last paragraph generated by flint
            self.widget.infocard.insertHtml(infocard)
        else:
            self.widget.infocard.insertHtml('<i>No infocard available.</i>')

        self.widget.infocard.insertHtml(f'<hr><small>nickname: {subject}</small>')
